pipeline {
  ...
  stages{
    ...
    stage('Testing'){
        steps{
            sh 'firebase deploy -P testingenvandressalguero --token "$FIREBASE_DEPLOY_TOKEN"' 
            script{
                try{
    ...
    stage('Staging'){
        ...
        steps{
          sh 'firebase deploy -P stagingdevopscctb --token "$FIREBASE_DEPLOY_TOKEN"'
        }
    }
    stage('Production'){
        steps{
               sh 'firebase deploy -P productiondevopscctb --token "$FIREBASE_DEPLOY_TOKEN"'
        }
    }
  }
}